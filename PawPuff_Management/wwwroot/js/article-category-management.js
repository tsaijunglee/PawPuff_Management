(() => {
    ...
    async function handleCreateSubmit(event) {
        ...
        try {
            // 先存進資料庫,成功後才把新的一列畫上去(用後端回傳的 id)
            const result = await postForm("/ArticleCategories/CreateAjax", { name });
            if (!result.success) {
                setCreateInvalid(result.message || "新增失敗。");
                return;
            }
    ...
    async function applyStatusChange(row, nextActive) {
        ...
        try {
            // 先存進資料庫,成功才更新畫面的開關
            const result = await postForm("/ArticleCategories/SetActiveAjax", { id, isActive: nextActive });
            if (!result.success) {
                showToast(result.message || "狀態更新失敗");
                return;
            }
