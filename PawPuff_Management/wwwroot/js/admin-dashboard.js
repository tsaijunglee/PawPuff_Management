...
function getNotificationAntiForgeryToken() {
    return document.querySelector(
        '#notificationAntiForgeryForm input[name="__RequestVerificationToken"]'
    )?.value || "";
}
...
function updateNotificationBadge(serverUnreadCount) {
    const domUnreadCount = document.querySelectorAll(
        "[data-notification].is-unread"
    ).length;
    const parsedServerCount = Number(serverUnreadCount);
    const unreadCount = Number.isInteger(parsedServerCount) &&
        parsedServerCount >= 0
        ? parsedServerCount
        : domUnreadCount;
...
function markNotificationReadInUi(item) {
    item.classList.remove("is-unread");
    item.classList.add("is-read");

    const notificationType =
        item.querySelector(".notification-meta span")?.textContent.trim() ||
        "通知";

    item.setAttribute("aria-label", `${notificationType}，已讀`);
}

async function markNotificationRead(item) {
    if (!item.classList.contains("is-unread")) return true;

    const notificationId = Number(item.dataset.notificationId);
    const markReadUrl = notificationCenter?.dataset.markReadUrl;
    const antiForgeryToken = getNotificationAntiForgeryToken();
    ...
    if (
        ...
        showNotificationToast("缺少通知編號或驗證資訊，請重新整理頁面。");
        return false;
    }
    ...
    try {
        ...
        if (!response.ok || payload.success !== true) {
            throw new Error(payload.message || "通知狀態更新失敗。");
        }

        markNotificationReadInUi(item);
        updateNotificationBadge(payload.unreadCount);
        return true;
    } catch (error) {
        console.error(error);
        showNotificationToast(
            error.message || "通知狀態更新失敗，請稍後再試。"
        );
        return false;
    } finally {
...
markAllRead?.addEventListener("click", async (event) => {
    ...
    const markAllReadUrl = notificationCenter?.dataset.markAllReadUrl;
    const antiForgeryToken = getNotificationAntiForgeryToken();

    if (!markAllReadUrl || !antiForgeryToken || markAllRead.disabled) {
        return;
    }
    ...
    try {
        ...
        document
            .querySelectorAll("[data-notification].is-unread")
            .forEach(markNotificationReadInUi);

        updateNotificationBadge(payload.unreadCount);
        showNotificationToast(payload.message || "所有通知已設為已讀。");
    } catch (error) {
        console.error(error);
        showNotificationToast(
            error.message || "通知狀態更新失敗，請稍後再試。"
...
document.querySelectorAll("[data-notification]").forEach((item) => {
    item.addEventListener("click", async (event) => {
        ...
        const updateSucceeded = await markNotificationRead(item);
        if (!updateSucceeded) return;

        const linkUrl = item.dataset.linkUrl?.trim() || "";

        setNotificationPanel(false);

        // link_url 為 null 時，Razor 會輸出空字串，因此只更新已讀，
        // 不執行任何頁面跳轉。
        if (linkUrl) {
