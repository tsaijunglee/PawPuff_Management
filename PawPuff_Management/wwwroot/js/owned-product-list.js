(() => {
  ...
  function getImagePath(name) {
    return name && !name.startsWith("#") ? "./assets/images/shop/" + name : "";
  }
  ...
  function renderThumb(row) {
    ...
    thumb.innerHTML = "";
    thumb.className = "shop-product-thumb" + (data.image.startsWith("#") ? " is-color" : "");
    thumb.style.removeProperty("--shop-color");

    const fallback = document.createElement("span");
    fallback.textContent = data.image;

    if (data.image.startsWith("#")) {
      thumb.style.setProperty("--shop-color", data.image);
  ...
  function collectRows() {
    rows = Array.from(refs.tableBody?.querySelectorAll("[data-owned-product-row]") || []);
  ...
  function updateSortIcons() {
    document.querySelectorAll("[data-owned-product-sort]").forEach((button) => {
      const icon = button.querySelector("i");
      if (!icon) return;

      icon.className = button.dataset.ownedProductSort === state.sortKey
  ...
  function renderTable() {
    ...
    refs.tableBody.querySelector("[data-owned-product-empty]")?.remove();
  ...
  function bindEvents() {
    document.querySelectorAll("[data-owned-product-filter]").forEach((input) => {
      const update = () => {
        state.filters[input.dataset.ownedProductFilter] = input.value.trim();
        state.page = 1;
        renderTable();
      };
  ...
  function init() {
    Object.assign(refs, {
      searchForm: document.getElementById("ownedProductSearchForm"),
      tableBody: document.getElementById("ownedProductTableBody"),
      pagination: document.getElementById("ownedProductPagination"),
      pageInfo: document.getElementById("ownedProductPageInfo"),
      resultCount: document.getElementById("ownedProductResultCount"),
      reset: document.getElementById("ownedProductFilterReset")
    });
