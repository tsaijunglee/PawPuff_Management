(() => {
  ...
  function renderDetail(row) {
    ...
    if (refs.shopDetailSortOrder) refs.shopDetailSortOrder.value = data.sortOrder === "NULL" ? "0" : data.sortOrder;
  ...
  function setStandaloneInvalid(input, error, visible) {
  ...
  function validateCreateForm() {
    ...
    if (currentCreateType === "配件") {
      ...
      if (!refs.shopCreateAccessorySort?.value) {
  ...
  function handleCreateSubmit(event) {
    ...
    refs.shopTableBody.appendChild(buildRow(buildProductFromCreate()));
    collectRows();
    state.sortKey = "id";
    state.sortDir = "asc";
